...
def graph_search(problem, fringe):
    ...
    cont = 0 #AGGIUNTO per calcolare quanti nodi ho visitato
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
            print("Nodos visitados = " + str(cont))
            return node
        cont = cont + 1
...
def ramificación_y_acotación(problem):
    return graph_search(problem, Cola())


def ramificación_y_acotación_con_subestimación (problem):
    return graph_search(problem, ColaConHeuristica(problem))
